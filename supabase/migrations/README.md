...
| `013_despublicar_versao.sql` | F7 | `despublicar_versao()`: devolve a versão ao rascunho para corrigir o que a primeira importação revelou. Trava: só passa quem nunca serviu histórico emitido. A vigência não é mexida, então as matrículas seguem ligadas a ela |
| `012_excluir_componente.sql` | F7 | `excluir_componente()`: exclusão numa transação só, com as checagens (currículo publicado, nota gravada) dentro da função. `componente` segue sem política de DELETE de propósito — é o que obriga a exclusão a passar por aqui |
