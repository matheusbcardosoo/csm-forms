...
// Perfil de acesso (usuario_perfil, migration 002). A RLS deixa cada usuário
// ler só a própria linha, então este select devolve no máximo 1 registro.
// null = e-mail sem perfil cadastrado (login funcionou, acesso negado).
//
// Filtra pelo e-mail da sessão: administradores enxergam TODAS as linhas
// (policy perfil_select_proprio_ou_admin), então sem o filtro o
// maybeSingle() falharia com "multiple rows".
async function getPerfil(client, email) {
  if (!email) {
  ...
  const { data, error } = await client
    .from('usuario_perfil')
    .select('email, nome, papel, ativo, criado_em, ultimo_acesso_em')
...
async function resolveSession(req, res) {
  ...
  try {
    ...
    if (!mustChangePassword) {
      perfil = await getPerfil(client, data.user.email);
      authorized = !!(perfil && perfil.ativo);
    }

    return { loggedIn: true, mustChangePassword, authorized, perfil: authorized ? perfil : null };
  } catch (_) {
    clearAuthCookies(res);
    return { loggedIn: false };
  }
...
module.exports = { getTokens, setAuthCookies, clearAuthCookies, requireAuth, resolveSession, getPerfil };
