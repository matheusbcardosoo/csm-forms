'use strict';
const puppeteer = require('puppeteer');

// Instancia unica do browser headless, reaproveitada entre chamadas
// (abrir/fechar o Chromium inteiro a cada PDF seria caro). Cada
// renderizacao usa sua propria aba (page), fechada ao final.
//
// Auto-recuperacao: se o Chromium morrer no meio do caminho (crash, OOM,
// processo matado pelo container), o evento 'disconnected' zera
// browserPromise — sem isso, a promise ficava resolvida pra sempre com um
// browser morto, e QUALQUER PDF (inclusive o modelo em branco) passava a
// falhar com "Connection closed." ate o processo Node ser reiniciado na
// mao. Com o cache zerado, a proxima chamada a getBrowser() relanca um
// Chromium novo sozinha.
let browserPromise = null;

function launchBrowser() {
  // Em producao (ver Dockerfile) usamos o Chromium do sistema instalado
  // via apt em vez do Chrome baixado pelo proprio Puppeteer — evita
  // depender de acesso de rede ao storage.googleapis.com durante o
  // build e problemas de libc incompativel (Alpine) em imagens Node
  // genericas. Localmente, sem essa env var, cai no Chrome que o
  // `npm install` do Puppeteer ja baixa normalmente.
  const executablePath = process.env.PUPPETEER_EXECUTABLE_PATH || undefined;
  const promise = puppeteer.launch({
    headless: true,
    executablePath,
    args: [
      '--no-sandbox',
      '--disable-setuid-sandbox',
      // Docker limita /dev/shm a 64MB por padrao — pouco pro Chromium, que
      // usa a area de memoria compartilhada pesadamente. Estourar esse
      // limite e a causa mais comum de o Chromium morrer sozinho dentro de
      // container (exatamente o "Connection closed." relatado), e essa
      // flag manda ele usar /tmp em vez de /dev/shm.
      '--disable-dev-shm-usage'
    ]
  });

  promise
    .then(browser => {
      browser.on('disconnected', () => {
        if (browserPromise === promise) browserPromise = null;
      });
    })
    .catch(() => {
      if (browserPromise === promise) browserPromise = null;
    });

  return promise;
}

function getBrowser() {
  if (!browserPromise) {
    browserPromise = launchBrowser();
  }
  return browserPromise;
}

// Erros tipicos de um Chromium que morreu no meio de uma renderizacao
// (crash, OOM) — nesses casos vale tentar relancar o browser e repetir a
// renderizacao uma vez, em vez de devolver erro pro usuario direto.
function isBrowserDeadError(err) {
  const msg = (err && err.message) || '';
  return /Connection closed|Target closed|Protocol error|Session closed|WebSocket is not open/i.test(msg);
}

// Folha A4 (210 x 297mm a 96dpi = 794 x 1123px).
const A4_WIDTH_PX = 794;
const A4_HEIGHT_PX = 1123;

// Abre uma URL interna (/internal/pdf/...) num browser headless e devolve
// o PDF como Buffer. Rotina compartilhada por qualquer PDF da aplicacao —
// tanto o de uma resposta preenchida (renderVisitaPdf) quanto o modelo em
// branco (renderVisitaBlankPdf) passam por aqui, entao os dois saem sempre
// com o mesmo tratamento de fontes/imagens/paginacao.
//
// opts.paginated: quando true, gera um PDF de verdade multi-pagina em
// formato A4 (o navegador quebra as paginas via CSS page-break, ver
// views/pdf-avaliacao.ejs) em vez do modo padrao abaixo, que mede a altura
// do conteudo e gera uma UNICA pagina customizada do tamanho exato — usado
// pra avaliacao substitutiva, onde cada aluno deve sair em sua propria
// pagina A4.
async function renderInternalPagePdfOnce(url, opts = {}) {
  const { paginated = false } = opts;
  const browser = await getBrowser();
  const page = await browser.newPage();
  try {
    // O browser e reaproveitado entre chamadas (getBrowser acima), entao seu
    // HTTP cache tambem persiste entre requisicoes. Gerando o mesmo PDF duas
    // vezes seguidas, a segunda navegacao virava um GET condicional que o
    // servidor respondia com 304 (ETag automatico do Express) — response.ok()
    // da false pra 304 e o goto() abaixo lancava erro mesmo com o conteudo
    // certo em cache.
    await page.setCacheEnabled(false);
    await page.setViewport({ width: A4_WIDTH_PX, height: A4_HEIGHT_PX });
    const response = await page.goto(url, { waitUntil: 'networkidle0', timeout: 20000 });
    if (!response || !response.ok()) {
      throw new Error(`Falha ao carregar pagina de PDF (status ${response && response.status()})`);
    }
    await page.waitForFunction('window.__PDF_READY__ === true', { timeout: 15000 });

    // __PDF_READY__ so garante que as imagens (logos) carregaram — as fontes
    // web (Google Fonts / FontAwesome, carregadas via <link> no <head>) podem
    // ainda estar chegando nesse momento. Se a altura for medida antes delas
    // terminarem, o texto reflui pra um line-height maior depois da medicao e
    // o conteudo passa a ultrapassar a altura fixada no PDF — foi isso que
    // jogou a assinatura pra uma segunda pagina em branco.
    await page.evaluate(() => (document.fonts && document.fonts.ready) || null);

    // page.pdf() usa o media type "print" por padrao. O site tem uma regra
    // @media print em public/css/styles.css (body * { visibility: hidden },
    // so mostra .print-document) pensada pra outro fluxo — isso deixava a
    // pagina inteira invisivel no PDF. Forcando "screen" aqui a gente ignora
    // esse @media print e renderiza com o mesmo CSS que aparece normalmente
    // no navegador.
    await page.emulateMediaType('screen');

    if (paginated) {
      // Uma pagina A4 "de verdade" por bloco (ver .pdf-page + page-break-after
      // no HTML renderizado) — se o conteudo de um bloco nao couber numa
      // folha, o navegador simplesmente continua na proxima em vez de cortar
      // (nunca perde conteudo, so deixa de ser exatamente "1 pagina por bloco").
      const pdfBytes = await page.pdf({
        format: 'A4',
        printBackground: true,
        margin: { top: '0px', bottom: '0px', left: '0px', right: '0px' }
      });
      return Buffer.from(pdfBytes);
    }

    // Na maioria dos casos o conteudo cabe dentro do A4, entao o PDF sai no
    // tamanho A4 "de verdade". So quando o conteudo e excepcionalmente longo
    // a altura cresce alem do A4 — preferimos isso a cortar conteudo ou
    // paginar.
    const contentHeightPx = await page.evaluate(() => {
      const root = document.getElementById('pdf-root');
      return Math.ceil(root ? root.scrollHeight : document.body.scrollHeight);
    });
    // +4px de folga contra arredondamento de subpixel entre a medicao e a
    // renderizacao final do PDF — sem isso, uma unica linha de 1px podia
    // transbordar pra uma segunda pagina.
    const pdfHeightPx = Math.max(contentHeightPx + 4, A4_HEIGHT_PX);

    const pdfBytes = await page.pdf({
      width: `${A4_WIDTH_PX}px`,
      height: `${pdfHeightPx}px`,
      printBackground: true,
      margin: { top: '0px', bottom: '0px', left: '0px', right: '0px' }
    });
    // Puppeteer 22+ devolve Uint8Array em vez de Buffer. res.send() do
    // Express so trata Buffer de verdade como binario — qualquer outro
    // objeto (Uint8Array incluso) ele serializa como JSON silenciosamente,
    // corrompendo o PDF. Convertendo aqui garante binario real em qualquer
    // lugar que consumir o retorno desta funcao.
    return Buffer.from(pdfBytes);
  } finally {
    await page.close();
  }
}

// Encapsula renderInternalPagePdfOnce() com uma tentativa extra: se o
// Chromium cacheado tiver morrido entre uma chamada e outra (ver
// launchBrowser/isBrowserDeadError acima), a primeira tentativa falha com
// "Connection closed." — aqui a gente forca um browser novo e tenta
// renderizar de novo antes de devolver erro pra quem chamou.
async function renderInternalPagePdf(url, opts = {}) {
  try {
    return await renderInternalPagePdfOnce(url, opts);
  } catch (err) {
    if (!isBrowserDeadError(err)) throw err;
    browserPromise = null;
    return await renderInternalPagePdfOnce(url, opts);
  }
}

// Renderiza a rota interna /internal/pdf/visita/:id (views/pdf-visita.ejs).
// Usada tanto pelo botao "Baixar PDF" (rota GET /api/responses/:id/pdf)
// quanto pelo envio automatico ao n8n (lib/n8n.js), garantindo que o
// arquivo seja sempre identico nos dois fluxos.
async function renderVisitaPdf({ baseUrl, visitaId, internalToken }) {
  if (!internalToken) {
    throw new Error('INTERNAL_PDF_SECRET nao configurado no .env');
  }
  const url = `${baseUrl}/internal/pdf/visita/${visitaId}?token=${encodeURIComponent(internalToken)}`;
  return renderInternalPagePdf(url);
}

// Gera o PDF do modelo de ficha de visita EM BRANCO (views/pdf-visita-blank.ejs)
// — sem dados de nenhuma visita especifica. Pensado pra secretaria imprimir
// e deixar disponivel na recepcao, pra quem prefere preencher a mao em vez
// do formulario online. Usa a mesma pipeline de renderInternalPagePdf().
async function renderVisitaBlankPdf({ baseUrl, internalToken }) {
  if (!internalToken) {
    throw new Error('INTERNAL_PDF_SECRET nao configurado no .env');
  }
  const url = `${baseUrl}/internal/pdf/visita-blank?token=${encodeURIComponent(internalToken)}`;
  return renderInternalPagePdf(url);
}

// Renderiza a rota interna /internal/pdf/avaliacao/:id (views/pdf-avaliacao.ejs).
// Mesmo papel de renderVisitaPdf() acima, so que pro requerimento de
// avaliacao substitutiva — usada tanto pelo botao "Baixar PDF" quanto pelo
// envio automatico ao n8n (lib/n8n.js). Paginado: cada aluno sai em sua
// propria pagina A4 (ver views/pdf-avaliacao.ejs), pra garantir a
// legibilidade dos anexos e permitir separar/entregar a pagina de um aluno
// especifico sem imprimir o requerimento inteiro.
async function renderAvaliacaoPdf({ baseUrl, avaliacaoId, internalToken }) {
  if (!internalToken) {
    throw new Error('INTERNAL_PDF_SECRET nao configurado no .env');
  }
  const url = `${baseUrl}/internal/pdf/avaliacao/${avaliacaoId}?token=${encodeURIComponent(internalToken)}`;
  return renderInternalPagePdf(url, { paginated: true });
}

// Modelo de requerimento de avaliacao substitutiva EM BRANCO, pra quem
// prefere preencher a mao. Mesmo papel de renderVisitaBlankPdf() acima.
async function renderAvaliacaoBlankPdf({ baseUrl, internalToken }) {
  if (!internalToken) {
    throw new Error('INTERNAL_PDF_SECRET nao configurado no .env');
  }
  const url = `${baseUrl}/internal/pdf/avaliacao-blank?token=${encodeURIComponent(internalToken)}`;
  return renderInternalPagePdf(url);
}

// Renderiza a rota interna /internal/pdf/historico/:id
// (views/pdf-historico.ejs) — o histórico escolar da fase 5. Paginado:
// o documento tem duas folhas A4 fixas (frente e verso), com quebra
// declarada no CSS compartilhado com a pre-visualizacao do painel.
async function renderHistoricoPdf({ baseUrl, historicoId, internalToken }) {
  if (!internalToken) {
    throw new Error('INTERNAL_PDF_SECRET nao configurado no .env');
  }
  const url = `${baseUrl}/internal/pdf/historico/${historicoId}?token=${encodeURIComponent(internalToken)}`;
  return renderInternalPagePdf(url, { paginated: true });
}

module.exports = {
  renderHistoricoPdf,
  renderVisitaPdf,
  renderVisitaBlankPdf,
  renderAvaliacaoPdf,
  renderAvaliacaoBlankPdf,
  getBrowser
};
