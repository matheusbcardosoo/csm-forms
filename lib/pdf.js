...
async function renderVisitaPdf({ baseUrl, visitaId, internalToken }) {
  ...
  try {
    ...
    // HTTP cache tambem persiste entre requisicoes. Ao gerar o PDF da mesma
    // visita duas vezes (ex: "Baixar PDF" e depois "Enviar por WhatsApp"),
    // a segunda navegacao virava um GET condicional que o servidor respondia
    // com 304 (ETag automatico do Express) — response.ok() da false pra 304
    // e o goto() abaixo lancava erro mesmo com o conteudo certo em cache.
